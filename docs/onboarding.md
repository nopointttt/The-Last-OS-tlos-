...
├── technical/
│   ├── hubs/
│   │   ├── documentation_hub.md   ← Технический хаб
│   │   └── product_hub.md         ← Продуктовый хаб (ссылки на vision)
│   ├── architecture/
│   │   └── architecture.md
│   ├── kernel/
│   │   ├── abi.md
...
| ADR-001.md                    | Архитекторы                           | Решение о выборе wasmCloud.                                                        | Accepted    | 10 мин      |
...
### Трек E: Возврат к проекту (когда прошло >3 месяцев)
1. onboarding.md (этот файл)
2. manifesto.md (напомнить ценности)
3. whitepaper.md (обновления визии)
4. ADR папка (новые решения)
5. documentation_hub.md → перейти к нужному разделу

---

## 5. Рекомендации по улучшению структуры

Текущая структура уже хороша (разделение на technical / vision / meta), но можно сделать ещё чище:

1. **Объединить хабы** в один главный `README.md` в корне `/docs`, который ссылается на этот onboarding.
2. **Добавить папку `roadmap/`** для текущих приоритетов и milestone-документов.
3. **Создать `glossary.md`** как отдельный файл (вынести из онбординга для удобного поиска).
4. **Ввести статусы документов** (Draft / Accepted / Deprecated) в фронтматтер каждого файла.
5. **Инструмент**: Перейти на **Mintlify** или **GitBook** — они отлично работают с Markdown и дают поиск + версионирование.
